/*
 * IS/STAG Web Services API
 *
 * Automaticky generovaná OpenAPI specifikace z WADL souborů pro webové služby IS/STAG.
 *
 * API version: 2.0.0
 * Contact: podpora@service.zcu.cz
 * Generated by: Swagger Codegen (https://github.com/swagger-api/swagger-codegen.git)
 */
package swagger

type CiselnikItemNewType struct {
	Key string `json:"key"`
	ValueCs string `json:"valueCs,omitempty"`
	ValueEn string `json:"valueEn,omitempty"`
	Valid bool `json:"valid"`
	Poradi int32 `json:"poradi"`
	Attributes []CiselnikItemNewAttributeType `json:"attributes,omitempty"`
}
